...
明天要做的是将每一条指令都解析一下，然后配合方法中的cfg（goto指令很需要这个），
进行污点分析，先将demo中的source点和sink点分析出来，然后再写一些其他的例子进行分析。
虚拟机需要做的事情：
1、识别指令，并找到相应的解释函数去解释执行。
执行的目的是：找到污点传播关系。寄存器对象有两个属性值，一个是污点值，一个是数值。以demo为例，
demo中污点进入数组后又取出来，然后赋值给一个变量，变量进入sink函数造成敏感信息泄露（or flaw）。
这里数组操作需要找到变量是被传到了哪个索引位置，并且对该索引位置进行标记而不是针对整体的数组。
先针对函数的识别：第一步找到函数invoke指令，第二步，找到返回指令，返回指令中的寄存器即为第一个污点寄存器。
下面关注该寄存器的污点传播情况。无非是赋值操作和函数调用，这里先不考虑函数调用，先考虑下赋值操作和数组操作。
关于赋值操作的寻址方式：
1、寄存器与寄存器之间的赋值操作就是两个寄存器之间的污点属性传播.
2、立即数，这个一般是针对数组操作的。锁定哪个数组成员是污点
3、而数组操作的关键步骤也是赋值操作，是将某个寄存器的赋给数组某个成员。这个阶段可以进行污点传播操作。
所以这里需要有一个数组对象。

miniDVM：
传入的callgraph，以及source点所在的函数。
初始化操作 初始化寄存器。从source点往下执行，直到找到sink点。
如果要增加指令解释支持，需要在goto_table中增加一个成员，然后再去实现键值对应的指令解释处理函数
对于操作码的提取，使用split函数分割空格取第一个成员。
'''
class miniDVM(object):
    def __init__(self):
        ...
        self.regTable=[]
        self.tainedFlag=False#如果找到一个source点则将这个tained点设置为True，然后开始找sink点
        ...
        self.goto_table={
            ...
            "sput-object": self.handle_aput,
            "sget-object": self.handle_aget,
            "sget-wide": self.handle_aget,
            "sput-wide": self.handle_aput,
            "sget-boolean": self.handle_aget,
            "sput-boolean": self.handle_aput,
            "sget-byte": self.handle_aget,
            "sput-byte": self.handle_aput,
            "sget-char": self.handle_aget,
            "sput-char": self.handle_aput,
            "sget-short": self.handle_aget,
            "sput-short": self.handle_aput,
    ...
    初始化dvm
    '''
    def initVM(self,activityEntryList, apkname,tmpl):
        sinkblocks=[]
        sourceblocks=[]
        entry_count=1
        ...
        for activityEntry in activityEntryList:
                ...
                if not isExists:
                    ...
                    cfgbuilder.imagepath=bytecodePath+"/images/"+encode_name
                else:
                    cfgbuilder.imagepath = bytecodePath + "/images/" + encode_name
                for tp in tmpl:
                    cfgbuilder.do_instructionflow(tp % activityEntry.encode("utf-8"))
                '''
                存储有向图Digraph
                '''
                ...
                firstblock = None
                for code in nodeset:
                    if "onCreate(Landroid/os/Bundle;)V 1\l" in code:
                        firstblock = code
                '''
